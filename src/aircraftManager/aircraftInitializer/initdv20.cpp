...
AircraftDefinition AircraftInitializer::initDV20()
{
    QVector<ColorZone> colorVec = { ColorZone{ .start = 950, .end = 2420, .color = QColor(0, 128, 0) }, ColorZone{ .start = 2420, .end = 2550, .color = QColor(255, 255, 0) }, ColorZone{ .start = 2550, .end = 2800, .color = QColor(255, 0, 0) } };
    ...
    colorVec = { ColorZone{ .start = 0.2 * 28.44, .end = 28.44, .color = QColor(0, 128, 0) }, ColorZone{ .start = 0.1 * 28.44, .end = 0.2 * 28.44, .color = QColor(255, 255, 0) }, ColorZone{ .start = 0, .end = 0.1 * 28.44, .color = QColor(255, 0, 0) } };
    ...
    GaugeDefinition fuelQtyGauge = RawGaugeDefinition{ .title = "L FUEL QTY R", .unitString = "KG", .minValue = 0, .maxValue = 28.44, .colorZones = colorVec, .grads = gradVec, .textGrads = textGradVec, .textIncrement = 1, .textNumDigits = 0, .forceTextColor = false, .textForcedColor = QColor(), .hasMinRedBlink = false, .minRedBlinkThreshold = 0, .hasMaxRedBlink = false, .maxRedBlinkThreshold = 0, .noText = false, .unit = Units::KG };


    colorVec = { };
    gradVec = { GradDef{ .gradPos = 0, .isBig = false, .gradColor = "white" }, GradDef{ .gradPos = 3, .isBig = false, .gradColor = "white" }, GradDef{ .gradPos = 6, .isBig = false, .gradColor = "white" }, GradDef{ .gradPos = 9, .isBig = false, .gradColor = "white" } };
    textGradVec = { TextGradDef{ .textGradPos = 0, .gradText = "0" }, TextGradDef{ .textGradPos = 3, .gradText = "3" }, TextGradDef{ .textGradPos = 6, .gradText = "6" }, TextGradDef{ .textGradPos = 9, .gradText = "9" }, TextGradDef{ .textGradPos = 12, .gradText = "12" } };

    GaugeDefinition fuelFlowGauge = RawGaugeDefinition{ .title = "FUEL FLOW", .unitString = "KGH", .minValue = 0, .maxValue = 12, .colorZones = colorVec, .grads = gradVec, .textGrads = textGradVec, .textIncrement = 1, .textNumDigits = 0, .forceTextColor = false, .textForcedColor = QColor(), .hasMinRedBlink = false, .minRedBlinkThreshold = 0, .hasMaxRedBlink = false, .maxRedBlinkThreshold = 0, .noText = false, .unit = Units::KG_PER_HOUR };


    colorVec = { ColorZone{ .start = 103.0 / 1.8, .end = 198.0 / 1.8, .color = QColor(0, 128, 0) }, ColorZone{ .start = 198.0 / 1.8, .end = 228.0 / 1.8, .color = QColor(255, 0, 0) } };
    gradVec = { };
    textGradVec = { };
    ...
    colorVec = { ColorZone{ .start = 0, .end = 90, .color = QColor(255, 0, 0) }, ColorZone{ .start = 90, .end = 150, .color = QColor(255, 255, 0) }, ColorZone{ .start = 150, .end = 500, .color = QColor(0, 128, 0) }, ColorZone{ .start = 500, .end = 650, .color = QColor(255, 255, 0) }, ColorZone{ .start = 650, .end = 700, .color = QColor(255, 0, 0) } };
    gradVec = { };
    textGradVec = { };
    ...
    AircraftDefinition ret{ .type = AircraftDefinition::AircraftType::PROP, .name = "DV20", .fuelQtyGauge = fuelQtyGauge, .fuelFlowGauge = fuelFlowGauge, .oilTempGauge = oilTempGauge, .oilPressGauge = oilPressGauge, .hasFlaps = true, .hasSpoilers = false, .hasElevatorTrim = true, .hasRudderTrim = true, .hasAileronTrim = true, .fuelQtyByVolume = false, .fuelFlowByVolume = false, .numEngines = 1, .numTanks = 2, .lowLimit = 0, .flapsBegin = 38, .flapsEnd = 81, .greenBegin = 43, .greenEnd = 117, .yellowBegin = 117, .yellowEnd = 161, .redBegin = 161, .redEnd = 180, .highLimit = 180, .noColors = false, .dynamicBarberpole = false };

    ret.currentType.setValue(PropDefinition(RawPropDefinition{ .rpmGauge = rpmGauge, .secondGauge = secondGauge, .egtGauge = egtGauge, .hasEgt = true, .usePropRpm = true, .secondIsLoad = true, .maxHp = 80 }));
