...
TscPageBase {
    ...
    Component.onCompleted: function() {
        currentInput = FlightTmr.getCurrentValue();
        updateInput();
    }
    ...
    function updateInput() {
        if (isInputing) {
            ...
            nonWriting.text = '<font color="white">' + '00'.slice(
                        0, 2 - hours.toString().length) + hours + ':'
                    + '00'.slice(0, 2 - minutes.toString(
                                     ).length) + minutes + ':' + '00'.slice(
                        0, 2 - seconds.toString().length) + seconds + '</font>'
            writing.text = ""
        }
    }
    ...
    function digitPressed(_digit) {
        ...
        currentInput = (10 * digits[0] + digits[1]) * 3600000 + (10 * digits[2] + digits[3])
                * 60000 + (10 * digits[4] + digits[5]) * 1000
